...
const VisitorMemo = ({ userId }) => {
  ...
  const [totalCount, setTotalCount] = useState(0); // non-deleted 방명록 개수
  ...
  useEffect(() => {
    async function fetchGuestbooks() {
      try {
        ...
        // 전체 댓글 개수 대신 non-deleted 항목의 개수 사용
        setTotalCount(freshMemos.length);

        // 댓글이 10개를 초과하면 마지막 페이지의 댓글 불러오기
        if (data.totalPages > 1) {
          const lastPage = data.totalPages - 1; // 프론트엔드에서는 0부터 시작하므로
